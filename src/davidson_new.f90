...
!    double precision::normloc
 !   double precision, external :: ddot
    ...
    nvec=molecule%davidson%nvecmin
    allocate(V(mesh%nactive,nvec))
    call davidson_basis_init_new(mesh,nvec,V,molecule)
    ...
    molecule%wf%epsprev(:)=0.0
    molecule%wf%deps(:)=0.0
    do while((iloop.le.molecule%cvg%loopmax).and.(cvg%ncvg.lt.cvg%nvec_to_cvg))
       write(*,'(A)') 'Main > #######################################'     
       write(*,'(A,I4,A)') 'Main > ############ scf loop=',iloop,' ############'
       write(*,'(A)') 'Main > #######################################'     
...
!       cvg%ETA=param%ETA
    ...
    !
    !--------------------------------------------------------------------------
    call save_wavefunction_new(mesh,V,molecule)
    deallocate(V)
    print *,"# Davidson > end of davidson subroutine"
  ...
  !
  ! --------------------------------------------------------------------------------------
    subroutine davidson_basis_init_new(mesh,nvec,V,molecule)
      implicit none
      type(t_mesh)::mesh
      integer::nvec
      double precision ,allocatable:: V(:,:) ! wavefunctions
      ...
      select case (molecule%davidson%init_wf)
      case (.TRUE.)
         print *,"Davidson > Initialisation of the wf"
         select case (molecule%davidson%restart)
         case (.TRUE.)
    ...
    end subroutine davidson_basis_init_new
  ! --------------------------------------------------------------------------------------
  !
  !              DAVIDSON_BASIS_INIT()
  !
